...
# Pathfinding Weights
COST_STEER = 0.5
COST_GEAR_SWITCH = 15.0  
COST_STEER_CHANGE = 1.0
H_WEIGHT = 2.5  

# Reeds-Shepp Constants
ANALYTIC_SHOT_DIST = 150.0  
RS_STEP_SIZE = 0.5
...
# --- NEW: Resample path to match manual generator spacing ---
def resample_segment(segment, step_cm):
    """Resamples a single directional segment linearly to exactly step_cm."""
    if len(segment) < 2:
        return segment
    
    out = [segment[0]]
    direction = segment[0][3]
    ...
    for i in range(len(segment)-1):
        ...
        while curr_dist + step_cm <= dist:
            ...
            yaw = yaw0 + t * diff
            steer = p0[4] + t * (p1[4] - p0[4])
            
            out.append((x, y, yaw, direction, steer))
            
        curr_dist -= dist # carry over remainder distance to next edge
        
    # CRITICAL: Always explicitly append the true endpoint (the cusp/goal)
    out.append(segment[-1])
    return out

def resample_path_with_gears(path, step_cm=2.0):
    """
    Groups path by direction (gear) and resamples each segment uniformly.
    This guarantees the exact same spatial density as your working manual tool.
    """
    if len(path) < 2:
        return path
        
    resampled = []
    current_dir = path[0][3]
    segment = [path[0]]
    ...
    for pt in path[1:]:
        if pt[3] == current_dir:
            ...
            resampled.extend(resample_segment(segment, step_cm))
            current_dir = pt[3]
            segment = [pt]
...
def save_path_json(path, start_pose, target_slot_id, target_pose, filename="planned_path.json"):
    # 1. Filter out physically impossible micro-maneuvers
    path = _filter_short_gear_islands_for_json(path, min_island_len_cm=5.0)
    
    # 2. NEW: Resample automated path to match manual drawing density (2.0 cm)
    # This prevents the Stanley window=5 from getting stuck at cusps!
    path = resample_path_with_gears(path, step_cm=2.0)
