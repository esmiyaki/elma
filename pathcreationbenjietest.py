...
# --- UPDATED Pathfinding Weights ---
COST_STEER = 0.5
COST_GEAR_SWITCH = 25.0  # Increased to strongly discourage cusps
COST_STEER_CHANGE = 5.0  # Increased to force smoother curves
H_WEIGHT = 2.5  
...
def _filter_short_gear_islands_for_json(
        ...
        *,
        min_island_len_cm: float = 10.0, # Updated to 10.0
...
def smooth_path_data(path, window=5):
    """Applies a moving average filter to yaw to prevent Stanley steering spikes."""
    if len(path) < window: return path
    
    smoothed_path = []
    for i in range(len(path)):
        x, y, yaw, d, steer = path[i]
        
        # Calculate moving average for yaw, being careful of angle wrapping
        sum_sin = 0
        sum_cos = 0
        count = 0
        
        # Only smooth points within the same gear/direction
        for j in range(max(0, i - window//2), min(len(path), i + window//2 + 1)):
            if path[j][3] == d: # Ensure same direction
                sum_sin += math.sin(path[j][2])
                sum_cos += math.cos(path[j][2])
                count += 1
                
        avg_yaw = math.atan2(sum_sin/count, sum_cos/count)
        smoothed_path.append((x, y, avg_yaw, d, steer))
        
    return smoothed_path

def save_path_json(path, start_pose, target_slot_id, target_pose, filename="planned_path.json"):
    path = _filter_short_gear_islands_for_json(path, min_island_len_cm=10.0)
    
    # NEW: Apply smoothing before exporting
    path = smooth_path_data(path)
...
def trim_path(full_path, min_tail_dist=20.0): # Increased from 15.0
