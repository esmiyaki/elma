...
def get_car_polygon(x, y, yaw, padding=0.0):
    # IMPORTANT: (x, y) is the localization reference point, NOT the car center.
    # Reference is 4cm inside from the FRONT bumper, so:
    #   - front overhang from reference: 4cm
    #   - rear overhang from reference: 21cm  (given CAR_L=25)
    front_overhang = 4.0 + padding
    rear_overhang = (CAR_L - 4.0) + padding
    w = CAR_W + (2 * padding)
    # Local frame: +X is forward, +Y is left (standard yaw usage in this file).
    outline = np.array([
        [-rear_overhang, -w / 2],
        [front_overhang, -w / 2],
        [front_overhang,  w / 2],
